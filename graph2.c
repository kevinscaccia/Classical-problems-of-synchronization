...
int graph_has_cicle( graph_t* graph, void *id_root ){
	node_t *root = find(graph, id_root);
	if( root == NULL){
		printf("ERRO 03");
		return FALSE;
	}
	edge_t *cursor = root->adj;
...
int main(){
	...
	graph_insert_edge(RESOURCE_GRAPH, &resources[2], &resources[0]);
	display(RESOURCE_GRAPH);
	int has = graph_has_cicle(RESOURCE_GRAPH, &resources[0]);
	if(has)
		printf("\nCOM ciclo\n");
	else
		printf("\nSEM ciclo\n");
